...
Build a production-ready financial reconciliation system for PretendCo using GitHub Copilot, Deno
TypeScript, and enterprise-grade quality enforcement. This capstone integrates all skills from
Courses 1-3 while demonstrating Toyota Way principles through PMAT quality gates.
...
1. **Plan Production Features** - Use GitHub Copilot to analyze requirements, query internal
   knowledge bases, and create comprehensive technical specifications
2. **Implement Full-Stack Solutions** - Build API layers, business logic, and data persistence with
   AI-assisted development
3. **Enforce Enterprise Quality** - Apply PMAT quality gates, achieve >80% test coverage, and
   maintain zero technical debt
4. **Deploy Confidently** - Create production-ready systems with monitoring, error handling, and
...
### Business Context (PretendCo)

PretendCo customers need to reconcile multi-currency invoices from international suppliers. The
system must:
...
// deno.json
{
  "name": "@pretendco/invoice-reconciliation",
  "version": "1.0.0",
  "tasks": {
...
1. **Business Problem** (0:00-1:00)
   - PretendCo customer pain point: manual invoice reconciliation
...
git clone https://github.com/pretendco-learning/invoice-reconciliation-starter
...
### Video 4.1.3: Understanding the PretendCo Context (3:30)

**Learning Objectives:**

- Navigate PretendCo's internal documentation
- Query knowledge bases using Copilot
- Understand PretendCo's API patterns and conventions

**Content:**

1. **PretendCo API Patterns** (0:00-1:15)
...
```
@workspace Search PretendCo documentation for:
...
```typescript
// Use Copilot to generate PretendCo-compliant types
// Prompt: "Generate TypeScript types for PretendCo invoice API based on docs/"

interface PretendCoInvoice {
...
export abstract class BaseRepository<T> implements Repository<T> {
  ...
  async findAll(filter?: Partial<T>): Promise<Result<T[], Error>> {
    try {
      const conditions = filter ? Object.keys(filter).map((k, i) => `${k} = $${i + 1}`) : [];
...
export function createTestInvoice(overrides?: Partial<Invoice>): Invoice {
  return {
    ...
    supplier: {
      ...
      taxId: `${faker.datatype.number({ min: 10, max: 99 })}-${
...
2. **Reference Implementations** (1:15-2:45)
   ...
   - Analyze PretendCo's public APIs
...
- [PretendCo API Documentation](https://developer.pretendco.com/documentation/)
...
_Authors: Noah Liam, Alfredo_\
_Partners: PretendCo & LinkedIn Learning_
