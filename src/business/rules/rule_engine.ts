...
export class ToleranceRule implements Rule<ReconciliationContext> {
  ...
  evaluate(context: ReconciliationContext): RuleResult {
    const expected = (context.purchaseOrder as unknown as Record<string, number>)[this.field];
    const actual = (context.invoice as unknown as Record<string, number>)[this.field];
...
export class RuleEngine {
  ...
  evaluate(context: ReconciliationContext): EvaluationResult {
