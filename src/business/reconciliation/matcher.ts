...
export class InvoiceMatcher {
  ...
  matchInvoice(
    ...
    const hasSufficientMatch = bestCandidate && bestCandidate.score.confidence >= this.CONFIDENCE_THRESHOLD;

    return {
      bestMatch: hasSufficientMatch ? bestCandidate.purchaseOrder : null,
      confidence: bestCandidate?.score.confidence ?? 0,
      alternatives: candidates.slice(1, 4),
      reasons: this.explainMatch(invoice, hasSufficientMatch ? bestCandidate : undefined),
    };
  }
